...
def main():
    ...
    parser.add_argument('-i', '--interface', default=conf.iface, help='Interface to use.')

    subparsers = parser.add_subparsers(dest='command', help='Command to perform.', required=True)

    discover_subparser = subparsers.add_parser('discover', help='Perform an ARP scan to discover endpoints.')
    discover_subparser.add_argument('IP', help='An IP address (e.g. 192.168.1.1) or address range (e.g. 192.168.1.1/24) to scan.')

    attack_subparser = subparsers.add_parser('attack', help='Perform an ARP poisoning MITM attack.')
    attack_subparser.add_argument('target', help='Target IP address.')
    attack_subparser.add_argument('gateway', help='Gateway IP address.')
